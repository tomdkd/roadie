...
export function SearchProjectModal({ isOpen, onClose, onSelectProject }: SearchProjectModalProps) {
  ...
  const filteredProjects = MOCK_PROJECTS.filter((project) => {
    const matchName = project.name.toLowerCase().includes(searchName.toLowerCase());
    const matchCity = project.city.toLowerCase().includes(searchCity.toLowerCase());
    const matchCountry = project.country.toLowerCase().includes(searchCountry.toLowerCase());
    const matchStyle =
      searchStyle === '' ||
      project.styles.some((s) => s.toLowerCase().includes(searchStyle.toLowerCase()));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-3xl bg-white p-6 shadow-2xl transition-all dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            ...
            filteredProjects.map((project) => (
              ...
              >
                ...
                <Button
                  ...
                  className="py-2 px-3 text-xs font-semibold rounded-xl flex items-center gap-1.5 shrink-0"
                >
        ...
        <div className="pt-3 border-t border-slate-100 dark:border-slate-800 text-right">
          <Button variant="outline" onClick={onClose} className="py-2 px-4 text-xs rounded-xl">
            Annuler
          </Button>
        </div>
      </div>
