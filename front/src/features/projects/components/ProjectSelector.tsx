...
export function ProjectSelector({ onOpenNewProjectModal }: ProjectSelectorProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedProject, setSelectedProject] = useState<ProjectOption>(MOCK_PROJECTS[0]);
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-blue-600 font-black text-xs text-white">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-full left-0 z-40 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-xl dark:border-slate-800 dark:bg-slate-900 animate-in fade-in zoom-in-95 duration-150">
            ...
            <button
              ...
              className="flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white transition-colors"
            >
