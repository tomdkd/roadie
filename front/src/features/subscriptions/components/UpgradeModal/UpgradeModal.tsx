...
export function UpgradeModal({ isOpen, onClose }: UpgradeModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:items-center sm:p-6 md:p-10">
      {/* Overlay Backdrop */}
      <div
      ...
      <div className="relative my-auto max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl transition-all sm:p-8 dark:border-slate-800 dark:bg-slate-900">
        {/* Halos décoratifs */}
        ...
        <button
          onClick={onClose}
          className="sticky top-0 z-10 float-right rounded-full bg-white/80 p-2 text-slate-400 backdrop-blur-md transition-colors hover:bg-slate-100 hover:text-slate-600 dark:bg-slate-900/80 dark:hover:bg-slate-800 dark:hover:text-slate-200"
        >
          <X className="h-5 w-5" />
        </button>

        {/* En-tête */}
        <div className="clear-both mx-auto max-w-lg pt-2 text-center sm:pt-0">
          ...
          <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
            Débloquez des fonctionnalités avancées pour piloter vos projets
            musicaux comme des pros.
          </p>
        </div>
        ...
        <div className="mt-6 flex items-center gap-3 rounded-2xl border border-purple-500/30 bg-purple-500/10 p-3.5 text-purple-900 dark:text-purple-200">
          ...
          <div className="text-xs">
            ...
            <p className="mt-0.5 leading-relaxed text-purple-700 dark:text-purple-300/80">
              Roadie est actuellement en version **Alpha**. En ayant reçu un
              accès anticipé, votre compte bénéficie de l'accès **Ultimate à vie
              (0€/mois)** pour vous remercier de nous aider à construire la
              plateforme.
            </p>
          </div>
        </div>
          ...
          {plans.map((plan) => {
            ...
            return (
              ...
              >
                {/* Badge Populaire */}
                {plan.popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 px-3 py-0.5 text-[10px] font-black tracking-wider text-white uppercase shadow-sm">
                    Recommandé
                  </span>
                )}
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`rounded-xl p-2 ${plan.badgeColor}`}>
