...
export function Sidebar({ isOpenMobile = false, onCloseMobile }: SidebarProps) {
  ...
  const navItems = [
    ...
    { to: '/messages', labelKey: 'nav.messages', defaultLabel: 'Messages', icon: MessageSquare },
    { to: '/documents', labelKey: 'nav.documents', defaultLabel: 'Documents', icon: FolderOpen },
  ];
  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-3 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-2.5">
          {/* Bouton Upgrade Pro */}
          <button
            type="button"
            onClick={() => {
            ...
            className={`w-full group relative flex items-center gap-2 rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 p-2.5 text-xs font-bold text-white shadow-md shadow-orange-500/20 hover:opacity-95 transition-all ${
              isCollapsed ? 'lg:justify-center lg:p-2.5' : 'justify-between'
            }`}
            title={isCollapsed ? 'Passer à la version Pro' : undefined}
          >
            <div className="flex items-center gap-2">
              <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white/20 backdrop-blur-sm text-white">
            ...
            {!isCollapsed && (
              <span className="rounded-full bg-white/20 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wider backdrop-blur-sm">
                Offre
              </span>
            )}
          </button>
          ...
          <div className={`flex items-center gap-2 ${isCollapsed ? 'lg:flex-col' : 'justify-center'}`}>
            <Link
              to="/help"
              onClick={onCloseMobile}
              className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200 transition-colors"
              title="Centre d'aide"
            >
            ...
            <a
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/40 dark:hover:text-red-400 transition-colors"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white transition-colors"
              title="Dépôt GitHub"
            >
